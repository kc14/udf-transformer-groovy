package com.kc14.guava.common.net;
...
public final class IDN {
  ...
  /**
   * Returns an instance of {@link IDN} (Internet Domain Name for short) after lenient validation. Specifically,
   ...
   */
  public static IDN from(String domain) {
    return new IDN(checkNotNull(domain));
  }
  ...
  IDN(String name) {
	...
	private int findPublicSuffix() {
		boolean strict = true;
		String assignedDomainName = EffectiveTldFinder.getAssignedDomain(name, strict);
		if (assignedDomainName == null) return NO_PUBLIC_SUFFIX_FOUND;
		return parts().size() - StringUtils.countMatches(assignedDomainName, ".") - 1;
	}
...
/**
  ...
  /**
   * Returns an {@code InternetDomainNameWithPSL} that is the immediate ancestor of this one; that is, the
   ...
   */
  private IDN ancestor(int levels) {
    return from(DOT_JOINER.join(parts.subList(levels, parts.size())));
  }

  /**
   * Creates and returns a new {@code InternetDomainNameWithPSL} by prepending the argument and a dot to
   * the current name. For example, {@code
   * InternetDomainNameWithPSL.from("foo.com").child("www.bar")} returns a new {@code InternetDomainNameWithPSL}
   ...
   */
  public IDN child(String leftParts) {
   ...
   *
   * <pre>   {@code
   *   domainName = InternetDomainNameWithPSL.isValid(name)
   *       ? InternetDomainNameWithPSL.from(name)
   *       : DEFAULT_DOMAIN;}</pre>
   *
   * <pre>   {@code
   *   try {
   *     domainName = InternetDomainNameWithPSL.from(name);
